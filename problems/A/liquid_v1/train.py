"""Multitask training entrypoint for the v1 bottle liquid model."""
...
from .datasets import LCDTCCropDataset, STATE_NAMES, TransparentObjectSegDataset
...
def compute_losses(
    ...
    if has_state.any():
        ...
        losses["state"] = F.cross_entropy(
            outputs["state_logits"][idx], state_labels, label_smoothing=args.label_smoothing
        )
...
def train(args: argparse.Namespace) -> None:
    ...
    lcd_loader = make_loader(lcd_train, args.lcd_batch, shuffle=True, workers=args.workers)
    lcd_val_loader = make_loader(lcd_val, args.lcd_batch, shuffle=False, workers=args.workers)
    seg_loader = (
        ...
        for lcd_batch in lcd_iter:
            ...
            if seg_iter is not None:
                ...
                with torch.amp.autocast("cuda", enabled=scaler.is_enabled()):
                    ...
                    seg_loss, seg_loss_dict = compute_losses(seg_outputs, seg_batch, args)
            ...
            if steps % args.print_interval == 0 or steps == len(lcd_loader):
                ...
                print(
                    ...
                    f"loss={avg['total']:.4f} state={avg['state']:.4f} binary={avg['binary']:.4f} "
                    f"bottle={avg['bottle']:.4f} area={avg['area']:.4f} lr={lr:.6f}"
                )
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser("Bottle liquid multitask training v1")
    parser.add_argument(
        "--lcdtc-root",
    ...
    parser.add_argument("--output-dir", default=str(here / "output_liquid_v1"))
    ...
    parser.add_argument("--lcd-batch", type=int, default=48)
