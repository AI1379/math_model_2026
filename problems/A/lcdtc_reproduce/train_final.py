...
def get_predictions(preds_raw, grids, num_cls, num_content):
    ...
    for i, (grid_xy, stride, gs) in enumerate(grids):
        ...
        all_stride.append(
            torch.full((n,), stride, device=device, dtype=torch.float32)
            .unsqueeze(0)
            .expand(B, -1)
        )
        ...
        all_content.append(
            raw[:, 5 + num_cls : 5 + num_cls + num_content]
            .view(B, num_content, n)
            .permute(0, 2, 1)
        )
...
def simota_assign(
    preds, gt_bboxes, gt_cls, gt_content, num_cls, num_content, fg_topk=10
):
    B, M = preds["obj"].shape
    ...
    for b in range(B):
        ...
        for gi in range(n_gt):
            ...
            tgt_dw[b, pos_idx] = (
                (g_w[gi] / s).clamp(min=1e-4).log().clamp(min=-10, max=10)
            )
            tgt_dh[b, pos_idx] = (
                (g_h[gi] / s).clamp(min=1e-4).log().clamp(min=-10, max=10)
            )
...
def compute_loss(
    preds_raw, targets, grids, num_cls=1, num_content=5, lambda_content=0.8
):
    device = preds_raw[0].device
    B = preds_raw[0].shape[0]
    ...
    loss_obj = F.binary_cross_entropy_with_logits(
        preds["obj"], obj_tgt, reduction="none"
    )
    alpha = 0.25
    weight = torch.where(
        obj_tgt > 0,
    ...
    loss_obj = (loss_obj * weight).sum() / (
        n_pos + alpha * (~pos).sum().float().clamp(min=1)
    )

    # Prepare advanced indexing for pos mask
    bb, aa = (
        pos.nonzero(as_tuple=True)
        if pos.any()
        else (torch.tensor([], dtype=torch.long), torch.tensor([], dtype=torch.long))
    )
    ...
    if pos.any():
        loss_cls = (
            F.binary_cross_entropy_with_logits(
                preds["cls"][bb, aa], cls_tgt[bb, aa], reduction="none"
            ).sum()
            / n_pos
        )
    ...
    if pos.any():
        p_dx, p_dy = preds["dx"][bb, aa], preds["dy"][bb, aa]
        p_dw, p_dh = (
            preds["dw"][bb, aa].clamp(max=10),
            preds["dh"][bb, aa].clamp(max=10),
        )
        stride_p = preds["stride"][bb, aa]
        gx_p, gy_p = preds["grid_xy"][bb, aa, 0], preds["grid_xy"][bb, aa, 1]
        p_cx = (gx_p + p_dx) * stride_p
        p_cy = (gy_p + p_dy) * stride_p
        p_w = p_dw.exp() * stride_p
        p_h = p_dh.exp() * stride_p
        pred_boxes = torch.stack(
            [p_cx - p_w / 2, p_cy - p_h / 2, p_cx + p_w / 2, p_cy + p_h / 2], dim=1
        )

        t_cx = (gx_p + tgt_dx[bb, aa]) * stride_p
        t_cy = (gy_p + tgt_dy[bb, aa]) * stride_p
        t_w = tgt_dw[bb, aa].exp() * stride_p
        t_h = tgt_dh[bb, aa].exp() * stride_p
        tgt_boxes = torch.stack(
            [t_cx - t_w / 2, t_cy - t_h / 2, t_cx + t_w / 2, t_cy + t_h / 2], dim=1
        )
    ...
    if pos.any():
        loss_content = (
            F.binary_cross_entropy_with_logits(
                preds["content"][bb, aa], cont_tgt[bb, aa], reduction="none"
            ).sum()
            / n_pos
        )
    ...
    return loss, {
        "obj": loss_obj.item(),
        "cls": loss_cls.item() if isinstance(loss_cls, torch.Tensor) else 0,
...
def decode_predictions(
    ...
    results = []
    for b in range(B):
        all_boxes, all_cls_scores, all_cont_scores, all_joint_scores, all_cont = [], [], [], [], []
        ...
        for i, (grid_xy, stride, gs) in enumerate(grids):
            ...
            cont_score, cont_label = content_score.max(1)
            ...
            cls_det_score = obj * cls_score
            cont_det_score = obj * cont_score
            joint_det_score = obj * cls_score * cont_score
            mask = joint_det_score > conf_thresh
            if mask.any():
                boxes = torch.stack(
                    [
                        cx[mask] - w[mask] / 2,
                ...
                all_cls_scores.append(cls_det_score[mask])
                all_cont_scores.append(cont_det_score[mask])
                all_joint_scores.append(joint_det_score[mask])
                all_cont.append(cont_label[mask])

        if all_boxes:
            all_boxes = torch.cat(all_boxes)
            all_cls_scores = torch.cat(all_cls_scores)
            all_cont_scores = torch.cat(all_cont_scores)
            all_joint_scores = torch.cat(all_joint_scores)
            all_cont = torch.cat(all_cont)

            if pre_nms_topk is not None and all_joint_scores.numel() > pre_nms_topk:
                topk_idx = all_joint_scores.argsort(descending=True)[:pre_nms_topk]
                all_boxes = all_boxes[topk_idx]
                all_cls_scores = all_cls_scores[topk_idx]
                all_cont_scores = all_cont_scores[topk_idx]
                all_joint_scores = all_joint_scores[topk_idx]
                all_cont = all_cont[topk_idx]

            keep_idx = batched_nms(all_boxes, all_joint_scores, all_cont, nms_thresh)
                ...
                results.append(
                    {
                        ...
                        "cls_scores": all_cls_scores[keep_idx],
                        "cont_scores": all_cont_scores[keep_idx],
                        "joint_scores": all_joint_scores[keep_idx],
...
def compute_ap(detections, gt_by_image, iou_thresh=0.5):
    ...
    for i, (img_idx, _score, det_box) in enumerate(detections):
        ...
        iou = _box_iou(det_box.unsqueeze(0), gt_boxes).squeeze(0).cpu()
        best_iou, best_j = iou.max(dim=0)
        if best_iou.item() >= iou_thresh and not matched[img_idx][best_j]:
            tp[i] = 1
...
def evaluate(
    ...
    all_dets = []
    with torch.no_grad():
        for batch_idx, (imgs, tgts, sizes) in enumerate(val_loader):
            imgs = imgs.to(device)
            ...
            if log_interval and (
                (batch_idx + 1) % log_interval == 0 or batch_idx + 1 == len(val_loader)
            ):
                print(f"  Eval [{batch_idx + 1}/{len(val_loader)}]")
    ...
    print(
        f"  APc@0.5={apc_05:.3f}  APt@0.5={apt_05:.3f}  APct@0.5={apct_05:.3f}  mAPct={mapct:.3f}"
    )
...
def _compute_combined_ap(all_dets, iou_thresh):
    ...
    for cid in range(5):
        detections = []
        gt_by_image = {}
        for img_idx, (det, gts) in enumerate(all_dets):
            gt_c = (
                torch.tensor(
                    [
                        [g["x1"], g["y1"], g["x2"], g["y2"]]
                ...
                if any(g["content"] == cid for g in gts)
                else torch.zeros(0, 4)
            )
            gt_by_image[img_idx] = gt_c
            if det is not None:
                m = det["cont_ids"] == cid
                if m.any():
                    boxes = det["boxes"][m].cpu()
                    scores = det["joint_scores"][m].cpu()
                    for j in range(boxes.shape[0]):
                        detections.append((img_idx, float(scores[j].item()), boxes[j]))
        aps.append(compute_ap(detections, gt_by_image, iou_thresh))
...
def _compute_category_ap(all_dets, iou_thresh):
    ...
    for img_idx, (det, gts) in enumerate(all_dets):
        ...
        if det is not None:
            boxes = det["boxes"].cpu()
            scores = det["cls_scores"].cpu()
            for j in range(boxes.shape[0]):
                detections.append((img_idx, float(scores[j].item()), boxes[j]))
    return compute_ap(detections, gt_by_image, iou_thresh)
...
class AugmentedLCDTCDataset(LCDTCDataset):
    ...
    def _load_mosaic(self, idx):
        ...
        if self.do_augment:
            mosaic, targets = self._augment(mosaic, targets)
...
class ModelEMA:
    ...
    def update(self, model):
        for k, v in model.state_dict().items():
            self.ema[k] = (
                self.decay * self.ema[k] + (1 - self.decay) * v.clone().detach()
            )
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % args.print_interval == 0:
            ...
            print(
                f"[{epoch + 1}/{args.epochs}] loss={avg_loss:.4f} {avg_detail} lr={lr:.6f} time={dt:.1f}s"
            )
        ...
        if (epoch + 1) % args.eval_interval == 0 or epoch == args.epochs - 1:
            ...
            metrics = evaluate(
                ...
                nms_thresh=args.eval_nms_thresh,
                pre_nms_topk=args.eval_pre_nms_topk,
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--amp", action="store_true", help="Enable mixed precision (fp16) training"
    )
    p.add_argument("--eval-interval", type=int, default=10)
    p.add_argument("--print-interval", type=int, default=5)
    p.add_argument("--eval-conf-thresh", type=float, default=0.001)
    p.add_argument("--eval-nms-thresh", type=float, default=0.65)
    p.add_argument("--eval-pre-nms-topk", type=int, default=1200)
    p.add_argument("--eval-max-det", type=int, default=100)
    p.add_argument("--eval-log-interval", type=int, default=10)
